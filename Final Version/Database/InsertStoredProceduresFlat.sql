...
Begin	
	if LEN(@AvailableFrom) > 0
		Update dbo.Flats set [AvailableFrom] = @AvailableFrom where [Id] = @FlatId

	if LEN(@DateOfOffer) > 0
		Update dbo.Flats set [DateOfOffer] = @DateOfOffer where [Id] = @FlatId

	if LEN(@Status) > 0
		Update dbo.Flats set [Status] = @Status where [Id] = @FlatId
End
Go
